...
console.log(
  `${colors.bright}node_modules exists:${colors.reset} ${hasNodeModules ? "Yes" : "No"}\n`
);
...
async function setup() {
  ...
  if (hasNodeModules) {
    console.log(
      `${colors.yellow}Removing existing node_modules for a clean installation...${colors.reset}`
    );
    const rmCmd = platform === "win32" ? "rmdir /s /q node_modules" : "rm -rf node_modules";
    if (!runCommand(rmCmd, "Cleaning node_modules")) {
      console.log(
        `${colors.yellow}Could not remove node_modules. Please delete it manually and try again.${colors.reset}`
      );
    }
  }
  ...
  if (fs.existsSync(path.join(__dirname, "..", "package-lock.json"))) {
    ...
    console.log(
      `${colors.yellow}No package-lock.json found, falling back to npm install${colors.reset}`
    );
    runCommand(
